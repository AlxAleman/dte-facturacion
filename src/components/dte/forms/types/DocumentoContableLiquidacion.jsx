// src/components/dte/forms/types/DocumentoContableLiquidacion.jsx
// Formulario específico para Documento Contable de Liquidación (Tipo 09)
...
function getInitialData() {
  ...
  return {
    identificacion: {
      ...
      numeroControl: "DTE-09-00000001-000000000000001",
      tipoModelo: 1,
      tipoOperacion: 1,
...
const DocumentoContableLiquidacion = ({ onDataChange, initialData }) => {
  const [formData, setFormData] = React.useState(initialData || getInitialData());
  const [isInitialized, setIsInitialized] = React.useState(false);
  const prevDataRef = React.useRef();

  // Campos requeridos específicos para Documento Contable de Liquidación
    ...
    <div className="space-y-6">
      <div className="bg-indigo-100 border border-indigo-300 rounded-lg p-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-xl font-bold text-indigo-900">📚 Documento Contable de Liquidación (Tipo 09)</h2>
            <p className="text-indigo-700">Documento para liquidaciones contables</p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white border border-gray-300 rounded-lg p-6">
        <h3 className="text-lg font-semibold text-gray-900 mb-4 flex items-center gap-2">
          ...
          Descripción de Liquidación y Observaciones
        </h3>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Descripción de Liquidación *</label>
            <input type="text" value={formData.extension.descripcionLiquidacion} onChange={e => handleExtensionChange('descripcionLiquidacion', e.target.value)} className={getFieldClassName('extension.descripcionLiquidacion')} placeholder="Descripción de la liquidación" />
          </div>
...
export default DocumentoContableLiquidacion; 