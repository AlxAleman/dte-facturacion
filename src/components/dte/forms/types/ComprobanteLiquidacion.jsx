// src/components/dte/forms/types/ComprobanteLiquidacion.jsx
// Formulario específico para Comprobante de Liquidación (Tipo 08)
...
function getInitialData() {
  ...
  return {
    identificacion: {
      ...
      numeroControl: "DTE-08-00000001-000000000000001",
      tipoModelo: 1,
      tipoOperacion: 1,
...
const ComprobanteLiquidacion = ({ onDataChange, initialData }) => {
  const [formData, setFormData] = React.useState(initialData || getInitialData());
  const [isInitialized, setIsInitialized] = React.useState(false);
  const prevDataRef = React.useRef();

  // Campos requeridos específicos para Comprobante de Liquidación
    ...
    <div className="space-y-6">
      <div className="bg-yellow-100 border border-yellow-300 rounded-lg p-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-xl font-bold text-yellow-900">🧾 Comprobante de Liquidación (Tipo 08)</h2>
            <p className="text-yellow-700">Documento para liquidaciones especiales</p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white border border-gray-300 rounded-lg p-6">
        <h3 className="text-lg font-semibold text-gray-900 mb-4 flex items-center gap-2">
          ...
          Motivo de Liquidación y Observaciones
        </h3>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Motivo de Liquidación *</label>
            <input type="text" value={formData.extension.motivoLiquidacion} onChange={e => handleExtensionChange('motivoLiquidacion', e.target.value)} className={getFieldClassName('extension.motivoLiquidacion')} placeholder="Motivo de la liquidación" />
          </div>
...
export default ComprobanteLiquidacion; 